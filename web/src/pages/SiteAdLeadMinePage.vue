...
async function queryMine() {
  ...
  try {
    ...
    queryForm.page = Number(json.data.page || queryForm.page)
    queryForm.pageSize = Number(json.data.pageSize || queryForm.pageSize)
    submitMsg.value = `共找到 ${total.value} 条投放单`
  } catch (error) {
...
function prevPage() {
  if (queryForm.page <= 1 || loading.value) return
  queryForm.page -= 1
  queryMine()
}

function nextPage() {
  const hasMore = queryForm.page * queryForm.pageSize < total.value
  ...
  <main class="lead-mine-page">
    ...
    <section class="card query-box">
      ...
      <div class="row actions">
        <label>
          每页条数
          <select v-model.number="queryForm.pageSize">
    ...
    <section class="card list-box">
      ...
      <div v-if="total > 0" class="row actions">
        <button class="btn" :disabled="queryForm.page <= 1 || loading" @click="prevPage">上一页</button>
        <button
          class="btn"
          :disabled="queryForm.page * queryForm.pageSize >= total || loading"
          @click="nextPage"
        >
          下一页
        </button>
        <span class="tip">第 {{ queryForm.page }} 页 / 共 {{ Math.max(Math.ceil(total / queryForm.pageSize), 1) }} 页</span>
      </div>
    </section>
  </main>
</template>
