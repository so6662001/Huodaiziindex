...
class N02IdentityControllerIntegrationTest {

  @Autowired private MockMvc mockMvc;

  @Test
  void optionsAndSwitchShouldWork() throws Exception {
    String loginResp =
        mockMvc
            .perform(
    ...
    mockMvc
        .perform(get("/api/v1/identity/options").header("X-Auth-Token", token))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.code").value("0"))
        .andExpect(jsonPath("$.data.currentIdentityCode").value("BUYER"))
    ...
    mockMvc
        ...
        .andExpect(jsonPath("$.data.message").value("身份已切换为 SUPPLIER"));
  }

  @Test
  void optionsShouldRejectWhenMissingToken() throws Exception {
    mockMvc
        .perform(get("/api/v1/identity/options"))
