...
class H5N05OrderDetailIntegrationTest {

  @Autowired private MockMvc mockMvc;

  @Test
  void h5OrderListDetailAndStatusShouldWork() throws Exception {
    mockMvc
        .perform(
    ...
    String listResp =
        mockMvc
            ...
            .andExpect(jsonPath("$.data.records[0].quickActionText").isNotEmpty())
            .andReturn()
            .getResponse()
            .getContentAsString();
    ...
    mockMvc
        .perform(get("/api/v1/auth/h5/orders/{orderId}", orderId).header("X-Auth-Token", token))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.code").value("0"))
    ...
    mockMvc
        .perform(
            post("/api/v1/auth/h5/orders/{orderId}/status", orderId)
                .header("X-Auth-Token", token)
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    """
                    {
                      "action":"MARK_PICKUP_IN_PROGRESS",
                      "remark":"H5端更新订单状态",
                      "operator":"h5-n05-test"
                    }
                    """))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.code").value("0"))
        .andExpect(jsonPath("$.data.orderStatus").value("PICKUP_IN_PROGRESS"))
        .andExpect(jsonPath("$.data.channel").value("H5"));
  }

  @Test
  void h5OrderListShouldRejectWhenMissingToken() throws Exception {
    mockMvc
        .perform(get("/api/v1/auth/h5/orders"))
