...
class N10CashierIntegrationTest {

  @Autowired private MockMvc mockMvc;

  @Test
  void cashierListDetailPayAndOrderStatusShouldWork() throws Exception {
    String loginResp =
        mockMvc
            .perform(
    ...
    mockMvc
        .perform(
            get("/api/v1/auth/cashier/orders/{cashierOrderId}", cashierOrderId)
                .header("X-Auth-Token", token))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.code").value("0"))
    ...
    mockMvc
        ...
        .andExpect(jsonPath("$.data.timeline[2].nodeCode").value("PAY_SUCCESS"));

    mockMvc
        .perform(get("/api/v1/auth/orders/{orderId}", orderId).header("X-Auth-Token", token))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.code").value("0"))
        .andExpect(jsonPath("$.data.orderId").value(orderId))
        .andExpect(jsonPath("$.data.orderStatus").value("COMPLETED"))
        .andExpect(jsonPath("$.data.reconcileStatus").value("PAID"));
  }
}
