...
public record A04PickupMonitorBatchUpdateRequest(
    @NotBlank(message = "contactMobile 不能为空")
    @Pattern(regexp = "^1\\d{10}$", message = "contactMobile 必须为11位手机号")
    String contactMobile,
    @NotEmpty(message = "pickupIds 不能为空")
    List<@NotBlank(message = "pickupId 不能为空") @Size(max = 64, message = "pickupId 最大长度64") String>
        pickupIds,
