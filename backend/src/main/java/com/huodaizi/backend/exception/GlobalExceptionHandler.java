...
public class GlobalExceptionHandler {
  ...
  public ResponseEntity<ApiResponse<Void>> handleBase(BaseException ex) {
    return ResponseEntity.status(toHttpStatus(ex.getCode())).body(ApiResponse.fail(ex.getCode(), ex.getMessage()));
  }
  ...
  public ResponseEntity<ApiResponse<Void>> handleBadRequest(Exception ex) {
    ...
    return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(ApiResponse.fail(ErrorCode.BAD_REQUEST, msg));
  }
