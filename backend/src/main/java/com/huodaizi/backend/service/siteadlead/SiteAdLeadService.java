...
public class SiteAdLeadService {
  ...
  public SiteAdLeadMineResponse mine(SiteAdLeadMineRequest request) {
    List<SiteAdLeadEntity> allMine = repository.mine(request);
    int page = request.safePage();
    int pageSize = request.safePageSize();
    int from = Math.max((page - 1) * pageSize, 0);
    int to = Math.min(from + pageSize, allMine.size());
    List<SiteAdLeadEntity> paged = from >= allMine.size() ? List.of() : allMine.subList(from, to);
    return new SiteAdLeadMineResponse(
        ...
        pageSize,
        allMine.size(),
        countByStatus(allMine, SiteAdLeadStatus.SUBMITTED),
        countByStatus(allMine, SiteAdLeadStatus.ASSIGNED),
        countByStatus(allMine, SiteAdLeadStatus.CONTACTED),
        countByStatus(allMine, SiteAdLeadStatus.PROPOSAL_SENT),
        countByStatus(allMine, SiteAdLeadStatus.CONVERTED),
        countByStatus(allMine, SiteAdLeadStatus.CLOSED));
  }
