...
public class InMemorySiteAdLeadRepository {
  ...
  public List<SiteAdLeadEntity> mine(SiteAdLeadMineRequest request) {
    String normalizedPhone = normalize(request.contactPhone());
    String keyword = normalize(request.keyword());
    List<SiteAdLeadEntity> filtered =
        store.values().stream()
            .filter(
                item ->
                    normalizedPhone == null
                        || item.getContactPhone().contains(normalizedPhone)
                        || item.getContactPhone().replaceAll("\\D", "").contains(normalizedPhone))
            .filter(
                item ->
                    keyword == null
  ...
  public List<SiteAdLeadEntity> adminList(SiteAdLeadListRequest request) {
    String status = normalizeStatusOrNull(request.status());
    String keyword = normalize(request.keyword());
